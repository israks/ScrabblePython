...
Jeu de Scrabble
Classes:
jeton- Enregistre la lettre du jeton et sa valeur
...
class Board:
    ...
    def ajoute_premium_squares(self):
        ...
        for coordinate in cases_MD:
            self.board[coordinate[0]][coordinate[1]] = "MD"
    ...
    def place_word(self, word, location, direction, player):
        ...
        global premium_spots # ce variable peut etre utiliser tout au long du code
        premium_spots = []
        ...
        for letter in word:
            for jeton in player.get_rack_arr():
                if jeton.get_letter() == letter:
                    player.rack.remove_from_rack(jeton)
...
class Bag:
    ...
    def add_to_bag(self, jeton, quantity):
        #Ajoute une certaine quantité d'un jeton spécifique. Prend comme argument un jeton et une quantité entière. 
        for i in range(quantity):
            self.bag.append(jeton)

    def initialize_bag(self):
        #Ajoute les 100 valeurs initiales à la pioche.
        global LETTER_VALUES
        self.add_to_bag(jeton("A", VAL_LETTRES), 9)
        self.add_to_bag(jeton("B", VAL_LETTRES), 2)
        self.add_to_bag(jeton("C", VAL_LETTRES), 2)
        self.add_to_bag(jeton("D", VAL_LETTRES), 4)
        self.add_to_bag(jeton("E", VAL_LETTRES), 12)
        self.add_to_bag(jeton("F", VAL_LETTRES), 2)
        self.add_to_bag(jeton("G", VAL_LETTRES), 3)
        self.add_to_bag(jeton("H", VAL_LETTRES), 2)
        self.add_to_bag(jeton("I", VAL_LETTRES), 9)
        self.add_to_bag(jeton("J", VAL_LETTRES), 9)
        self.add_to_bag(jeton("K", VAL_LETTRES), 1)
        self.add_to_bag(jeton("L", VAL_LETTRES), 4)
        self.add_to_bag(jeton("M", VAL_LETTRES), 2)
        self.add_to_bag(jeton("N", VAL_LETTRES), 6)
        self.add_to_bag(jeton("O", VAL_LETTRES), 8)
        self.add_to_bag(jeton("P", VAL_LETTRES), 2)
        self.add_to_bag(jeton("Q", VAL_LETTRES), 1)
        self.add_to_bag(jeton("R", VAL_LETTRES), 6)
        self.add_to_bag(jeton("S", VAL_LETTRES), 4)
        self.add_to_bag(jeton("T", VAL_LETTRES), 6)
        self.add_to_bag(jeton("U", VAL_LETTRES), 4)
        self.add_to_bag(jeton("V", VAL_LETTRES), 2)
        self.add_to_bag(jeton("W", VAL_LETTRES), 2)
        self.add_to_bag(jeton("X", VAL_LETTRES), 1)
        self.add_to_bag(jeton("Y", VAL_LETTRES), 2)
        self.add_to_bag(jeton("Z", VAL_LETTRES), 1)
        self.add_to_bag(jeton("?", VAL_LETTRES), 2)
        shuffle(self.bag)
...
class Rack:
    ...
    def replenish_rack(self):
        ...
        while self.get_rack_length() < 7 and self.bag.get_remaining_jetons() > 0:
            self.add_to_rack()
...
class Word:
    ...
    def check_word(self):
        ...
        if "dictionary" not in globals():
            dictionary = open("liste_mots.txt").read().splitlines()
        ...
        if self.word != "":
            ...
            if "?" in self.word:
                while len(blank_jeton_val) != 1:
                    blank_jeton_val = input("Veuillez saisir la lettre que représente le joker: ")
                self.word = self.word[:word.index("?")] + blank_jeton_val.upper() + self.word[(word.index("?")+1):]
            ...
            #Déclenche une erreur si le mot en cours de lecture n'est pas dans le dictionnaire officiel du scrabble (liste_mots.txt).
            #on a ici une erreur. On sais que ce bout de code est problematique. Quand on essaye avec le not in dictionary qui devrait logiquement marcher, on a une erreur. On a donc enlever le not que pour qu'on puisse continuer a tester le code
            ...
            for i in range(len(self.word)):
                if current_board_ltr[i] == " ":
                    needed_jetons += self.word[i]
                elif current_board_ltr[i] != self.word[i]:
                    print("Current_board_ltr: " + str(current_board_ltr) + ", Word: " + self.word + ", Needed_jetons: " + needed_jetons)
            ...
            if blank_jeton_val != "":
                needed_jetons = needed_jetons[needed_jetons.index(blank_jeton_val):] + needed_jetons[:needed_jetons.index(blank_jeton_val)]
            ...
            for letter in needed_jetons:
                if letter not in self.player.get_rack_str() or self.player.get_rack_str().count(letter) < needed_jetons.count(letter):
    ...
    def valeur_mot(self):
        ...
        for letter in self.word:
            for spot in premium_spots:
                if letter == spot[0]:
                    if spot[1] == "LT":
                        word_score += VAL_LETTRES[letter] * 2
                    elif spot[1] == "LD":
                        word_score += VAL_LETTRES[letter]
            word_score += VAL_LETTRES[letter]
        for spot in premium_spots:
            if spot[1] == "MT":
                word_score *= 3
...
def turn(player, board, bag):
    ...
    if (skipped_turns < 6) or (player.rack.get_rack_length() == 0 and bag.get_remaining_jetons() == 0):
